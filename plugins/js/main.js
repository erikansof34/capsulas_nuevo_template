...
async function loadSlider(index) {
  ...
  try {
    ...
    // Controlar visibilidad del botón next solo en el último slider
    const btnNext = document.getElementById('next');
    if (btnNext) {
      if (index === sliders.length - 1) {
        // Último slider: ocultar hasta completar actividad
...
window.nextSlide = () => {
  if (currentIndex < sliders.length - 1) {
    ...
    // Estamos en el último slide, verificar si se completó la actividad
    if (localStorage.getItem('actividad_guardada') === 'true') {
      navegarAPaginaSiguiente();
    } else {
      alert('Debes completar y guardar la actividad antes de continuar');
    }
  }
};

// Funciones para controlar la navegación desde la actividad
window.habilitarNavegacionEvaluacion = function() {
  localStorage.setItem('actividad_guardada', 'true');
  // Mostrar el botón next si estamos en el último slider
  if (currentIndex === sliders.length - 1) {
    const btnNext = document.getElementById('next');
    if (btnNext) {
      btnNext.style.display = 'block';
...
window.deshabilitarNext = function() {
  localStorage.removeItem('actividad_guardada');
  const btnNext = document.getElementById('next');
  if (btnNext && currentIndex === sliders.length - 1) {
    btnNext.style.display = 'none';
  }
};

window.habilitarNext = function() {
  const btnNext = document.getElementById('next');
  if (btnNext && currentIndex === sliders.length - 1) {
    btnNext.style.display = 'block';
    btnNext.style.opacity = '1';
